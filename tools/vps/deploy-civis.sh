...
if [ -n "$HUBVH" ] && [ -f "$HUBVH" ]; then
  ...
  if [ -n "$PREVROOT" ] && [ -f "$PREVROOT/config.php" ] \
     && grep -q "'site_slug' => 'civismedia'" "$PREVROOT/config.php"; then
    PREV="$PREVROOT/config.php"
...
' "$SRC/config.php" "$REL/config.php" "$PREV" || fail "couldn't write the hub config"
