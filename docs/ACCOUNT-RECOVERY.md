...
| Command | Use it when |
| --- | --- |
| `list` | You want to see which accounts exist, who is an admin — and whether the throttle is currently holding anyone. |
...
| `clear-2fa --all` | Housekeeping. Two-step sign-in has been removed; this wipes the secrets it left behind so nobody is enrolled to a lost phone if it ever returns. |
...
| `hub-check` | A hub page 403s or comes up empty and you want to know which gate is shut. |
...
Three gates stand between an administrator and a page at civismedia.ca,
and from the browser they look much alike. `hub-check` reads all three:
...
3. **The role itself** — `require_admin()` answers 403 to an editor.

There used to be a fourth, and it was the one that bit: two-step
sign-in, required of hub administrators by default, funnelling anyone
not yet enrolled to the profile page and letting them reach nothing
...
`totp.cleared`, `role.changed`, `account.created`, `security.unfenced`
