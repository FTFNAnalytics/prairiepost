...
/* --- The migration journal (owned by tools/migrate.php) ------------------ */

/**
 * The journal is created by the RUNNER under its lock, not by a numbered
 * migration — adoption of an existing database has to record itself before
 * any step can run. Rows: one 'adopted'/'installed' baseline, then one row
 * per step, written 'started' before the step and finished after it (on
 * engines with transactional DDL, in the same transaction as the DDL).
 */
function pp_journal_ddl(string $driver): string
{
...
function pp_journal_exists(PDO $pdo, string $driver): bool
{
    try {
        if ($driver === 'pgsql') {
            $stmt = $pdo->prepare('SELECT 1 FROM pg_tables WHERE schemaname = ? AND tablename = ?');
            $stmt->execute([pp_pg_schema(), 'schema_migrations']);
            return $stmt->fetch() !== false;
        }
        $sql = $driver === 'mysql'
...
/**
 * The exact source text of one migration step, for checksumming: a journal
 * row records the sha256 of the step AS APPLIED, so a later edit to an
 * already-applied step is detectable instead of silent.
 */
function pp_migration_source(int $version): string
{
    $fn = pp_migrations()[$version] ?? null;
    if ($fn === null) {
        return '';
    }
...
function pp_migration_checksum(int $version): string
{
    return hash('sha256', pp_migration_source($version));
}

/** A short human label for a step: its first comment line, if it has one. */
function pp_migration_label(int $version): string
{
    if (preg_match('#//\s*(.+)#', pp_migration_source($version), $m)) {
        return mb_substr(trim($m[1]), 0, 180);
    }
    return 'schema step ' . $version;
}

/**
 * Non-mutating readiness classification for a connected target.
 *   ready    installed, at exactly PP_SCHEMA_VERSION, no step mid-flight
 *   empty    the app schema is not installed here at all
 *   behind   installed at an older version — run the migration runner
 *   ahead    installed at a NEWER version than this code understands
 *   partial  a journal row says a step started and never finished
 */
function pp_schema_status(PDO $pdo, string $driver): array
{
    if (!pp_schema_installed($pdo, $driver)) {
        return ['state' => 'empty', 'version' => null, 'target' => PP_SCHEMA_VERSION];
    }
    $v = pp_schema_version($pdo);
    if (pp_journal_exists($pdo, $driver)) {
        $open = (int) $pdo->query("SELECT COUNT(*) FROM schema_migrations WHERE status = 'started'")->fetchColumn();
        if ($open > 0) {
            return ['state' => 'partial', 'version' => $v, 'target' => PP_SCHEMA_VERSION];
        }
    }
    if ($v < PP_SCHEMA_VERSION) {
        return ['state' => 'behind', 'version' => $v, 'target' => PP_SCHEMA_VERSION];
    }
    if ($v > PP_SCHEMA_VERSION) {
        return ['state' => 'ahead', 'version' => $v, 'target' => PP_SCHEMA_VERSION];
    }
    return ['state' => 'ready', 'version' => $v, 'target' => PP_SCHEMA_VERSION];
}

/**
 * Every schema migration, one entry per target version, in order. The
 * bodies are the former pp_migrate() ladder blocks, moved verbatim —
 * each still writes its own settings version bump as its final act, so
 * the stored version always reflects the last COMPLETED step. The
 * runner (tools/migrate.php) executes these one at a time under a lock
 * and journals each step; nothing in the request path calls them.
 */
function pp_migrations(): array
{
    return [
        2 => function (PDO $pdo, string $driver): void {
            $dt = $driver === 'pgsql' ? 'TIMESTAMP' : 'DATETIME';
        ...
        3 => function (PDO $pdo, string $driver): void {
            $dt = $driver === 'pgsql' ? 'TIMESTAMP' : 'DATETIME';
            $id = $driver === 'mysql' ? 'INT UNSIGNED AUTO_INCREMENT PRIMARY KEY'
                : ($driver === 'pgsql' ? 'INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY' : 'INTEGER PRIMARY KEY AUTOINCREMENT');
        ...
        4 => function (PDO $pdo, string $driver): void {
            $dt = $driver === 'pgsql' ? 'TIMESTAMP' : 'DATETIME';
            $id = $driver === 'mysql' ? 'INT UNSIGNED AUTO_INCREMENT PRIMARY KEY'
                : ($driver === 'pgsql' ? 'INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY' : 'INTEGER PRIMARY KEY AUTOINCREMENT');
        ...
        5 => function (PDO $pdo, string $driver): void {
        ...
        6 => function (PDO $pdo, string $driver): void {
        ...
        7 => function (PDO $pdo, string $driver): void {
        ...
        8 => function (PDO $pdo, string $driver): void {
        ...
        9 => function (PDO $pdo, string $driver): void {
        ...
        10 => function (PDO $pdo, string $driver): void {
        ...
        11 => function (PDO $pdo, string $driver): void {
        ...
        12 => function (PDO $pdo, string $driver): void {
        ...
        13 => function (PDO $pdo, string $driver): void {
        ...
        14 => function (PDO $pdo, string $driver): void {
        ...
        15 => function (PDO $pdo, string $driver): void {
        ...
        16 => function (PDO $pdo, string $driver): void {
        ...
        17 => function (PDO $pdo, string $driver): void {
        ...
        18 => function (PDO $pdo, string $driver): void {
        ...
        19 => function (PDO $pdo, string $driver): void {
            $dt = $driver === 'pgsql' ? 'TIMESTAMP' : 'DATETIME';
            $id = $driver === 'mysql' ? 'INT UNSIGNED AUTO_INCREMENT PRIMARY KEY'
                : ($driver === 'pgsql' ? 'INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY' : 'INTEGER PRIMARY KEY AUTOINCREMENT');
